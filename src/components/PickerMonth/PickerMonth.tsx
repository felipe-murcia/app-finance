...
const PickerMonth: React.FC<MonthYearPickerModalProps> = ({ visible, onClose, onConfirm, month = new Date().getMonth() + 1, year = new Date().getFullYear() }) => {
  ...
  const renderItem = ({ item }: { item: { label: string; value: number } }) => (
    <TouchableOpacity 
    style={[styles.itemContainer, item.value == selectedMonth ? {backgroundColor:colorBlack}:null]} 
    onPress={()=>setSelectedMonth(item.value)}>
      <Text style={[styles.itemText, item.value == selectedMonth ? { color:"white"}:null]}>{item.label}</Text>
    </TouchableOpacity>
  );
      ...
      <View style={styles.modalBackground}>
        <View style={styles.modalContainer}>
          ...
          <TouchableOpacity onPress={()=>{onConfirm({ month: selectedMonth, year: selectedYear }); onClose(); }} >
              <View style={{backgroundColor:colorBlack, borderRadius: 8, padding:8, alignItems:'center', paddingHorizontal:20, flex:1}} >
